...
class UI(ft.Container):
                ...
                controls=[
                    ft.Row(
                        controls=[
                            ...
                            ft.Icon(ft.Icons.CIRCLE, color=ft.Colors.GREEN, size=10),
                        ],
                        spacing=10,
                    ),
                    ft.Row(
    ...
    def _initialize_ui_components(self):
        ...
        self.uma_instance = UMA(
            ...
            page=self.page,
            reloj_global=self.reloj_global  # <-- PASAR RELOJ_GLOBAL
        )
...
################################### PAGUINA 1 (UMA) ####################################
        self.home_container_1 = ft.Container(
            ...
            content=ft.Column(
                expand=True,
                controls=[
                    self.uma_instance,  # UMA maneja toda la interfaz
                ]
            )
        )
...
################################### PAGUINA 4 (ALERTAS) ####################################
        ...
        # ¡AÑADE ESTA LÍNEA! - Esto hará que el botón UMA aparezca activo al inicio
    ...
    def actualizar_colores_botones(self, index_activo):
        ...
        botones = [self.btn_connect, self.btn_connect2, self.btn_connect4, self.btn_connect3]
        
        # Guardar los colores originales para referencia
        self.colores_originales = [ft.Colors.AMBER, ft.Colors.BLUE, ft.Colors.GREEN, ft.Colors.RED]
        ...
        for i, btn in enumerate(botones):
            if i == index_activo:
                ...
                btn.disabled = True
                # Guardar que este es el botón activo
                self.boton_activo_actual = btn
                # Sombra verde para resaltar
...
def main(page: ft.Page):
    ...
    page.window.height = 700  # Aumenté un poco la altura
    ...
    page.padding = 0  # Cambiado a 0 para mejor control

    # No crear UI inmediatamente, el login lo hará
    ui = UI(page)
    # No agregar a la página aquí, el login lo manejará
